# Django settings for beagle ide project.
import os
import sys
...
SESSION_COOKIE_NAME = 'beaglesession'
...
HTTP_PORT = 7000
IMG_EDITOR_PORT = 7001

DJANGO_SERVE_STATIC_MEDIA = False

BG_IMG = 'ide/img/beagle.jpg'

try:
  from beagle.settings_local import *
...
if not globals().has_key('WORKING_DIR'):
  WORKING_DIR = os.path.join(SPATH, '..', 'files')
...
if not globals().has_key('LOGPATH'):
  if DEBUG:
    LOGPATH = os.path.join(LOG_DIR, 'beagleide_debug.log')
    
  else:
    LOGPATH = os.path.join(LOG_DIR, 'beagleide.log')
  
if not globals().has_key('PKLPATH'):
  PKLPATH = os.path.join(WORKING_DIR, 'beagleide.pkl')
...
if not globals().has_key('DATABASES'):
  DATABASES = {
      'default': {
          ...
          'NAME': os.path.join(WORKING_DIR, 'beagle.sql3'),                      # Or path to database file if using sqlite3.

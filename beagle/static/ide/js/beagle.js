...
function SaveCurrentTab () {
  ...
  $("#status").html('');
//tab_paths[dp].filename
  $("#status").html('Kaydedildi.');
  $('#status').fadeIn().delay(1400).fadeOut('slow');
  ...
  $.ajax({
    ...
    error: function (jqXHR, textStatus, errorThrown) { alert('Kayit Hatasi: ' + dp); $("#status").html(''); },
  });
}
...
function Find () {
  $("#find").html('');
  $("#find").html('Ara...');
  $("#find").fadeIn().delay(1400).fadeOut('slow');
}
...
function resize_editor (skip_splitter) {
  var edith = $(window).height() - 48;
  
  ksplitter.size("#beagle_body", edith + "px");
  $("#splitter, #splitter > div").height(edith - 2);
  
  var nbw = $("#beagle_body").width() + 1;
  $("#beagle_body").width(nbw);
...
function remove_tab (ui) {
  ...
  if (count == 1) {
    editor_global.setValue("Yeni Bir Dosya Aciniz.");
  }
...
function get_file (file, range) {
  ...
  if (img_patt.test(file)) {
    ...
    $.ajax({
      ...
      error: function (jqXHR, textStatus, errorThrown) { alert('Dosya Acilamiyor ' + file); },
    });
  }
}
...
$(document).ready(function () {
  ...
  ksplitter = $("#beagle_ui").kendoSplitter({
    panes: [{resizable: false, size: '50px', scrollable: false}, {scrollable:false, resizable: false, size: '311px'}],
...
function add_commands (e) {
  ...
  e.commands.addCommand({
    ...
    exec: function(env, args, request) { Find(); }
  });
}
